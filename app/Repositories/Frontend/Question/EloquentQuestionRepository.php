...
use App\Exceptions\Frontend\Access\Question\QuestionNeedsOrganizationsException;
...
class EloquentQuestionRepository implements QuestionContract {
	 ...
	 */
	public function create($input, $project_id) {
	...
	public function update($id, $input, $project) {
		$question = $this->findOrThrowException($id, true); //print_r($input); print_r($question);die();
		$related_id = $input['related_data']['q'];
                if(!empty($related_id)){
                ...
                //$question->save();
                //$toUpdate = $question->whereQnum($input['qnum'])->whereProjectId($project['project_id'])->first();
                //dd($toUpdate);
		if ($question->whereQnum($input['qnum'])->whereProjectId($project['project_id'])->first()->update($input)) { 
		//	$this->flushProject($project['project_id'], $question);
